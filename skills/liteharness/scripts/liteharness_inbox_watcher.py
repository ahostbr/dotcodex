...
PID_PATH = STATE_ROOT / "codex_inbox_watcher.pid"
HEARTBEAT_PATH = STATE_ROOT / "codex_inbox_watcher.heartbeat.json"
...
def load_target() -> tuple[int, int] | None:
...
def process_incoming_messages(agent_id: str) -> int:
    ...
    target = load_target()
    if not target:
        append_log("no headed target captured; skipping standalone injection scan")
        return 0
    ...
    for message in fetch_targeted_messages(agent_id):
        ...
        if inject_message_into_target(claimed, target):
            inbox.complete(claimed, result="auto-injected-into-standalone-terminal")
            delivered += 1
        else:
