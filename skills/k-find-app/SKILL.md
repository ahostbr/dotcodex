...
If `$ARGUMENTS` is empty, use `request_user_input` when available, or ask directly in chat, to ask:
...
Present the top matches using `request_user_input` when available, or a concise plain-text choice list when it is not:
