...
Fetch, format, and archive YouTube transcripts using `yt-dlp`. Transcripts are automatically saved to both The Nexus Prismatica database and LiteSuite's YouTube database (so they appear in the YouTube panel).
...
2. **Run the script**:
   ```bash
   powershell.exe -ExecutionPolicy Bypass -File "C:/Users/Ryan/.codex/skills/youtube-transcript/Get-YouTubeTranscript.ps1" -Url "VIDEO_URL"
   ```
   If the user specified an output path, add `-OutputPath "PATH"`:
   ```bash
   powershell.exe -ExecutionPolicy Bypass -File "C:/Users/Ryan/.codex/skills/youtube-transcript/Get-YouTubeTranscript.ps1" -Url "VIDEO_URL" -OutputPath "PATH"
   ```
